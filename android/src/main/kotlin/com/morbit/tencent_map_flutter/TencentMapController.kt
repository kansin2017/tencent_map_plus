  ...
  private fun onMethodCall(call: MethodCall, result: MethodChannel.Result) {
    when (call.method) {
      ...
      "goSearch"->{
        ...
        if (keyword != null) {
          api.goSearchByKeyword(keyword).thenAccept { res ->
            if (res != null) {
              Log.i("搜索结果", res.toString())
              result.success(res)
            } else {
              Log.e("搜索失败", "没有搜到")
              result.error("SEARCH_FAILED", "没有搜到结果", null)
            }
          }.exceptionally { throwable ->
            Log.e("搜索异常", "异常信息：", throwable)
            result.error("SEARCH_ERROR", "搜索异常", throwable)
            null
          }
        } else {
          result.error("INVALID_ARGUMENT", "keyword 参数为空", null)
        }
      }
